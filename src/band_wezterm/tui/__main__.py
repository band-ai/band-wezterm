...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--agent-action",
        choices=tuple(action.value for action in InitialAgentAction),
        default=InitialAgentAction.BROWSE.value,
    )
    parser.add_argument(
        "--screen",
...
def main(argv: list[str] | None = None) -> int:
    ...
    return run_control_app(
        ...
        initial_agent_action=InitialAgentAction(args.agent_action),
        initial_agent_id=args.agent_id,
    )
